...
class Simulator_Behavior(object):
    ...
    def simulate_experiment(self):
        ...
        for b in torch.arange(self.nb):
            ...
            for t in torch.arange(self.nt):
                ...
                agent.update_beliefs(b, t, condition=int(condition), strategy=int(strategy), question=int(question), reaction_time=reaction_time, error = error)
                if agent.d_con:
                    if ~torch.isnan(agent.evi_d_list[-1]):
                        self.lglike_sum += torch.log(agent.evi_d_list[-1])
                        self.num_d += 1
                if agent.e_con:
                    if ~torch.isnan(agent.evi_e_list[-1]):
                        self.lglike_sum += torch.log(agent.evi_e_list[-1])
                        self.num_e += 1
        ...
        self.lglike_mean = self.lglike_sum / (self.num_d + self.num_e)
